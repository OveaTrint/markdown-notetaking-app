...
def test_check_endpoint_accepts_markdown_file(mocker):
    ...
    mock_file = io.BytesIO(MD_content)
...
def test_save_endpoint_with_valid_input(tmp_path, monkeypatch: pytest.MonkeyPatch):
...
def test_save_endpoint_with_empty_input(tmp_path, monkeypatch):
    monkeypatch.setattr("main.markdown_dir", tmp_path)
    content = ""
...
def test_save_endpoint_with_non_utf8_characters(tmp_path, monkeypatch):
    monkeypatch.setattr("main.markdown_dir", tmp_path)
...
def test_notes_endpoint_with_no_saved_notes(tmp_path, monkeypatch):
    monkeypatch.setattr("main.markdown_dir", tmp_path)
...
def test_notes_endpoint_with_saved_notes(tmp_path, monkeypatch):
    monkeypatch.setattr("main.markdown_dir", tmp_path)
